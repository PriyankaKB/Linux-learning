# Basics of Linux

Linux is a family of open-source Unix-like operating systems based on the Linux kernel. It is widely used for servers, desktops, embedded systems, and more. Linux is known for its stability, security, and flexibility. It supports multitasking, multiuser operations, and a wide range of hardware platforms.

## Key Features
- Open-source and free to use
- Highly customizable
- Strong community support
- Secure and stable
- Supports a wide range of software and hardware

## Common Linux Distributions
- Ubuntu
- Debian
- Fedora
- CentOS
- Red Hat Enterprise Linux (RHEL)
- Arch Linux

---

# Basic Linux Commands

| Command         | Description                                             |
|----------------|---------------------------------------------------------|
| `ls`           | List directory contents.                                |
| `cd`           | Change the current directory.                           |
| `pwd`          | Print the current working directory.                    |
| `mkdir`        | Create a new directory.                                 |
| `rmdir`        | Remove empty directories.                               |
| `rm`           | Remove files or directories.                            |
| `cp`           | Copy files or directories.                              |
| `mv`           | Move or rename files or directories.                    |
| `touch`        | Create an empty file or update file timestamps.         |
| `ln`           | Create hard and symbolic links.                         |
| `cat`          | Concatenate and display file contents.                  |
| `tac`          | Display file contents in reverse.                       |
| `nl`           | Number the lines of files.                              |
| `more`         | View file contents one page at a time.                  |
| `less`         | View file contents with backward/forward navigation.    |
| `head`         | Display the first lines of a file.                      |
| `tail`         | Display the last lines of a file.                       |
| `echo`         | Display a line of text.                                 |
| `printf`       | Format and print data.                                  |
| `grep`         | Search for patterns in files.                           |
| `find`         | Search for files and directories.                       |
| `locate`       | Find files by name quickly.                             |
| `which`        | Show the full path of shell commands.                   |
| `whereis`      | Locate the binary, source, and manual page for a command.|
| `man`          | Display the manual for a command.                       |
| `info`         | Display command information in info format.             |
| `history`      | Show command history.                                   |
| `clear`        | Clear the terminal screen.                              |
| `chmod`        | Change file permissions.                                |
| `chown`        | Change file ownership.                                  |
| `chgrp`        | Change group ownership.                                 |
| `df`           | Display disk space usage.                               |
| `du`           | Display directory space usage.                          |
| `free`         | Show memory usage.                                      |
| `top`          | Display real-time process information.                  |
| `htop`         | Interactive process viewer (if installed).              |
| `ps`           | Display running processes.                              |
| `kill`         | Terminate a process by PID.                             |
| `killall`      | Terminate processes by name.                            |
| `bg`           | Resume a job in the background.                         |
| `fg`           | Bring a job to the foreground.                          |
| `jobs`         | List active jobs.                                       |
| `uname`        | Print system information.                               |
| `hostname`     | Show or set the system hostname.                        |
| `date`         | Display or set the system date and time.                |
| `cal`          | Display a calendar.                                     |
| `whoami`       | Show the current user.                                  |
| `id`           | Show user and group information.                        |
| `passwd`       | Change user password.                                   |
| `su`           | Switch user.                                            |
| `sudo`         | Execute a command as another user.                      |
| `apt`/`yum`/`dnf` | Package management commands (Debian/RedHat/Fedora).   |
| `snap`         | Manage snap packages.                                   |
| `tar`          | Archive files.                                          |
| `zip`/`unzip`  | Compress and decompress files.                          |
| `gzip`/`gunzip`| Compress and decompress files with gzip.                |
| `bzip2`/`bunzip2`| Compress and decompress files with bzip2.              |
| `scp`          | Securely copy files between systems.                    |
| `rsync`        | Synchronize files and directories.                      |
| `ssh`          | Connect to a remote server.                             |
| `ftp`/`sftp`   | Transfer files via FTP/SFTP.                            |
| `wget`         | Download files from the web.                            |
| `curl`         | Transfer data from or to a server.                      |
| `ifconfig`     | Display network configuration.                          |
| `ip`           | Show/manipulate routing, devices, policy routing, etc.  |
| `ping`         | Test network connectivity.                              |
| `traceroute`   | Trace the route to a host.                              |
| `netstat`      | Network connections, routing tables, interface stats.   |
| `ss`           | Another utility to investigate sockets.                 |
| `route`        | Show/manipulate the IP routing table.                   |
| `mount`        | Mount a filesystem.                                     |
| `umount`       | Unmount a filesystem.                                   |
| `df`           | Report file system disk space usage.                    |
| `fdisk`        | Partition table manipulator.                            |
| `mkfs`         | Build a Linux file system.                              |
| `fsck`         | File system consistency check and repair.               |
| `crontab`      | Schedule periodic jobs.                                 |
| `at`           | Schedule a command to run once at a certain time.       |
| `alias`        | Create an alias for a command.                          |
| `unalias`      | Remove an alias.                                        |
| `export`       | Set environment variables.                              |
| `env`          | Show environment variables.                             |
| `sort`         | Sort lines of text files.                               |
| `uniq`         | Report or omit repeated lines.                          |
| `cut`          | Remove sections from each line of files.                |
| `awk`          | Pattern scanning and processing language.               |
| `sed`          | Stream editor for filtering and transforming text.      |
| `diff`         | Compare files line by line.                             |
| `patch`        | Apply a diff file to an original.                       |
| `wc`           | Print newline, word, and byte counts for each file.     |
| `tee`          | Read from standard input and write to standard output and files. |
| `xargs`        | Build and execute command lines from standard input.    |
| `sleep`        | Delay for a specified amount of time.                   |
| `shutdown`     | Bring the system down.                                  |
| `reboot`       | Reboot the system.                                      |
| `logout`       | Exit a login shell.                                     |
| `exit`         | Exit the shell.                                         |
